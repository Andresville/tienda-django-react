...
const SellerForm = () => {
  ...
  const fetchSeller = async (sellerId) => {
    try {
      ...
      // Filtra para encontrar el seller a editar (la API de listado no tiene un endpoint GET por ID)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/admin/sellers'); // Redirige a la lista
    } catch (err) {
  ...
  return (
    <div className="card p-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Link to="/admin/sellers" className="btn btn-secondary ms-2">Cancelar</Link>
      </form>
    </div>
  );
};
